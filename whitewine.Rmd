...
```{r fig.align='center',cache=FALSE}
sugar.log <- wine$residual.sugar %>% log
figure(title= "Distribution of Log Transform of residual sugar", 
       xlab =TeX('Log Transform of Residual Sugar g/$dm^3$')) %>%
  ly_hist(sugar.log,breaks = 40, freq = FALSE, 
...
ggplot(data=wine, xlab="Quality", ylab="Alcohol [%]",
  ...
  labs(data=wine,x="quality",y="Alcohol [%]") + 
...
ggplot(data=wine,aes(x = wine[,1],
  ...
  ylab(TeX('Density g/$cm^3$'))+ggtitle("Scatter Plot",)
```
